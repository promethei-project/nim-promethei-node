...
suite "merkletree - coders":
  test "encoding and decoding a tree yields the same tree":
    let
      tree = PrometheiTree.init(Sha256HashCodec, data).tryGet()
      encodedBytes = tree.encode()
      decodedTree = PrometheiTree.decode(encodedBytes).tryGet()
  ...
  test "encoding and decoding a proof yields the same proof":
    ...
    let
      ...
      decodedProof = PrometheiProof.decode(encodedBytes).tryGet()
