...
All node metrics should be prefixed with the `promethei_` prefix to be able to differentiate from metrics exposed by other subsystems. For example libp2p generally prefixed with the `libp2p_` prefix.
...
This directory contains both the default `prometheus.yml` config file as well as a basic `promethei-grafana-dashboard.json` file that can be augmented with additional panels and metrics on an as needed basis.
...
Use either one of the presented methods (upload json, load from a url or copy paste the json into the text-box), to upload the `promethei-grafana-dashboard.json` file.
